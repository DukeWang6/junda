...
$("#addSysteminfo").click("click", function() {
	addSysteminfo();
});
$("#editSysteminfo").click("click", function() {
	editSysteminfo();
});
$("#delSysteminfo").click("click", function() {
	delSysteminfo();
});
...
function addSysteminfo() {
	pageii = layer.open({
		...
		type : 2,
		area : [ "600px", "80%" ],
		content : rootPath + '/systeminfo/addUI.shtml'
	});
}
...
function editSysteminfo() {
	...
	pageii = layer.open({
		...
		type : 2,
		area : [ "600px", "80%" ],
		content : rootPath + '/systeminfo/editUI.shtml?id=' + cbox
	});
}
function delSysteminfo() {
	var cbox = grid.getSelectedCheckbox();
	if (cbox == "") {
		layer.msg("请选择删除项！！");
		return;
	...
	layer.confirm('是否删除？', function(index) {
		var url = rootPath + '/systeminfo/deleteEntity.shtml';
